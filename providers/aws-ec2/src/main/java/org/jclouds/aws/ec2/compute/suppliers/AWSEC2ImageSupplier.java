...
public class AWSEC2ImageSupplier implements Supplier<Set<? extends Image>> {
   ...
   public Set<? extends Image> get() {
      ...
      cache.get().asMap().putAll((Map)imageMap); // TODO
      logger.debug("<< images(%d)", imageMap.size());
      
      logger.info("AWSRegionAndNameToImageSupplier retrieved "+imageMap.size()+" images in "+(System.currentTimeMillis()-startTime)+"ms");
